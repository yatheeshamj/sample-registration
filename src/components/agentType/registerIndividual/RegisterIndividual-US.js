...
class RegisterIndividualUS extends Component {
            ...
            case "businessinfo":
                return ((<Translate>
                    {({ translate }) => <Fragment>
                        ...
                        />
                        <ConfirmModal
                            title={`${translate(`${CURRENT_SCREEN}.registerAsSoleProprietor.heading`)}`}
                            isVisible={true}
                            onHide={() => { }}
                            hideCancel={true}
                            hideOk={true}
    ...
    renderSelectPathChangeContainer = () => {
        ...
        const headerTitle = "Don’t want to register as a Sole Proprietor?";
                    ...
                    <Translate>
                        {({ translate }) => <>
                            <div className={classNames({
                                ...
                                {/* <br /> */}
                                {this.currentSubstep == "ssn" && <h1 className={`${commonStyle['heading_1']} ${commonStyle['heading_1_topUS']} ${commonStyle['regularFont']} ${commonStyle['componentsBottomMargin']} `} style={{ textTransform: 'uppercase' }}>{translate(`${CURRENT_SCREEN}.registerAsSoleProprietor.sideHeader`)}</h1>}
    ...
    render() {
        ...
        if (selectPathSteps && !selectPathSteps.completed) {
            ...
            if (incomingSubStep !== this.currentSubstep)
                return <Redirect to={{ pathname: ADMISSION_STEP_ROUTES.soleProprietor_us, search: `?step=${incomingSubStep}` }} />;
                            ...
                            <Fragment>
                                <div className={`row `}>
                                    <div className={classNames({
                                        ...
                                        {/* {this.currentSubstep === "ssn" && <br />} */}
                                        <h1 className={`${commonStyle['heading_1']} ${commonStyle['regularFont']} ${commonStyle['heading_1_top']} ${commonStyle['active_color']} ${commonStyle['widthChange4']}  ${styles['lineHeight']} `}>{translate(`${CURRENT_SCREEN}.heading`)}</h1>
                                        {this.currentSubstep === "ssn" && <p className={`${commonStyle['widthChange']} ${commonStyle['paddingChangeMedium']} ${commonStyle['lightFont']} ${commonStyle['blackColor']} ${commonStyle['paragraph_3']} ${commonStyle['componentsMargin']}`}>{translate(`${CURRENT_SCREEN}.description`)}</p>}
