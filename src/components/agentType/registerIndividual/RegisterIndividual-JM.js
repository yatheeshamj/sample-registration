...
class RegisterIndividualJM extends Component {
            ...
            case "businessinfo":
                return ((<Translate>
                    {({ translate }) => <Fragment>
                        ...
                        />
                        <ConfirmModal
                            title={`${translate("Register as a Sole Trader")}`}
                            isVisible={true}
                            onHide={() => { }}
                            hideCancel={true}
                            hideOk={true}
                    ...
                    <Translate>
                        {({ translate }) => <>
                            <div className={classNames({
                                ...
                                {/* {this.currentSubstep == "ssn" && <h1 className={`${commonStyle['heading_1']} ${commonStyle['heading_1_topUS']} ${commonStyle['regularFont']} ${commonStyle['componentsBottomMargin']} `} style={{ textTransform: 'uppercase' }}>{headerTitle}</h1>}
    ...
    render() {
        ...
        if (selectPathSteps && !selectPathSteps.completed) {
            ...
            const headerTitle = "Register as a Sole Trader";
            const headerSubTitle = "soleTraderCopy"

            if (incomingSubStep !== this.currentSubstep) {
                return <Redirect to={{ pathname: ADMISSION_STEP_ROUTES.soleProprietor_jm, search: `?step=${incomingSubStep}` }} />;
            }
