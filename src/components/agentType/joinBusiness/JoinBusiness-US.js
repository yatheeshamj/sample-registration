...
class JoinBusiness extends Component {
      ...
      if (
        ...
        if (
          ...
          childStep.childSteps.some(
            (x) => x.uniqueId == AdmissionStep.JB_VERIFY_IDENTITY && x.completed
          ) &&
          childStep.childSteps.some(
            (x) =>
  ...
  setReferralStatus = (currentStep, childStep) => {
    ...
    } else if (currentStep == "sendrequest") {
  ...
  renderHeaderSubTitle = (substeps) => {
    ...
    if (agentType.joinBusiness.statusId == JoinCallCenter.PENDING_ACCEPTANCE)
    ...
    return (
      <Translate>
        {({ translate }) => (
          <>
            <div
              className={`d-none d-lg-block col-lg-4 ${styles["align-center"]} ${styles["horizontal-spacing"]}`}
            >
  ...
  renderSelectPathChangeContainer = () => {
    ...
    const headertitle = "Don't want to join a Service Partner?";
    ...
    if (selectPathSteps) {
      ...
      if (agentTypeSteps && agentProfile) {
        const filterdAgentTypeSteps = agentTypeSteps.filter(
              ...
              <>
                <div
                  className={classNames({
                    ...
                    "offset-lg-1": this.currentSubstep == "ssn",
                    "offset-lg-2":
                      this.currentSubstep == "businessinfo" ||
                      this.currentSubstep == "pendingfinalization" ||
  ...
  render() {
    ...
    if (selectPathSteps && !selectPathSteps.completed) {
      ...
      return incomingSubStep !== this.currentSubstep ? (
        <Redirect
          to={{
            pathname: ADMISSION_STEP_ROUTES.joinServicePartner_us,
              ...
              <Fragment>
                <div className={`row`}>
                  <div
                    className={classNames({
                      "col-lg-5":
                        this.currentSubstep !== "agreements" && !referVerify,
                      "col-lg-7":
                        this.currentSubstep == "agreements" && !referVerify,
                      "col-md-10 offset-md-1 offset-lg-1 col-xs-12":
                        true && !referVerify,
                      "col-lg-7":
                        this.currentSubstep !== "agreements" && referVerify,
                      "col-lg-7":
                  ...
                  >
                    ...
                    {!isUKUser && this.currentSubstep == "agreements" && (
                      <p
                  ...
                  {!referVerify && this.renderSelectPathChangeContainer()}
                </div>
