...
const RadioButtonHorizontally = ({
    ...
    return (
        <div className="form-group">
            ...
            {/* Flexbox container with inline style */}
            <div style={{ display: 'flex', justifyContent: 'start' }}>
                {options.map((o, key) => (
                    <div key={key} className="custom-control custom-radio" style={{ marginRight: '15px' }}>
                        <input
                            id={`${o.label}-${o.value}-${id}`}
                            name={field.name}
                            type="radio"
                            ...
                            {...field}
                            {...props}
                            onChange={(e) => setFieldValue(field.name, o.value)}
                        />
...
export default RadioButtonHorizontally;
